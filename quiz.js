...
/*Question order*/
var quiz_order = [job_tree, health_concerns_tree, health_visits_tree, edu_tree, transport_tree, park_tree, library_tree, grocery_tree];
...
$(function () {
    ...
    var renderList = function (items) {
        ...
        for (var i = 0; i < items.length; i++) {
            var item = items[i];
            $list.append('<li><a id = "quiz-question" href="#" data-choice="' + item.id + '">' + item.name + '</a></li>');
        }
    };
    ...
    var _nextQuestions = function (currentTree) {
        for(var i = 0; i < quiz_order.length-1; i++){
            if(String(currentTree) == String(quiz_order[i].name)){
                console.log("MATCH");
                tree = new DecisionTree(quiz_order[i + 1]);
                var initData = tree.getInitial();
                current_id = null;
                renderList(initData);
                console.log("NEXT QUESTIONS: " + tree.name);
            }
        }
    };
    ...
    $(document).on('click', '#choices a', function (e) {
        ...
        if (check_endpoint == 'yes') {
            ...
            var currentTree = tree.getChoice(choiceId).current;
            _nextQuestions(currentTree);
        }
    });
